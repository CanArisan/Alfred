...
class RescaleTransform:
    ...
    def __call__(self, images):
        scaling_factor = (float(self.max) - float(self.min)) / (float(self._data_max) - float(self._data_min))
        scaling_disposition = float(self.min) - float(self._data_min)
        images = images * scaling_factor + scaling_disposition
        return images
    ...
    def scale_values(self, values):
        ...
        result = []
        for v in values:
            result.append(v * scaling_factor + scaling_disposition)
        return result
...
def compute_image_mean_and_std(images):
    ...
    """
    mean = np.mean(images, axis=(0, 1, 2), dtype=np.float64)
    std = np.std(images, axis=(0, 1, 2), dtype=np.float64)
    return mean, std
